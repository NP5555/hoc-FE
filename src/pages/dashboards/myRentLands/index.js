          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell align='center'>Pay Rent</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {data.slice().map(row => {
                if (row?.tenant?.id !== state.reducer.userData.userData.user.id || row.isOnchain !== true) {
                  return null
                }
                ...
                return (
                  <TableRow key={row.id}>
                    <TableCell align='center'>
                      <Link href={`/rent-land-detail?land=${row.id}`} passHref>
                        <a>
                    ...
                    {row.isVacationRent ? (
                      <TableCell align='center'>{row.lastPaymentTime}</TableCell>
                    ) : (
                    ...
                    {row.isVacationRent ? (
                      <>
                        <TableCell align='center'>
                          <Button onClick={() => confirmRentLand(row)} variant='outlined'>
                            <Icon fontSize='1.125rem' icon='cryptocurrency:pay' />
                          </Button>
                        </TableCell>
                      </>
                    ) : (
                      <>
                        <TableCell align='center'>
                          <Button onClick={() => confirmRentLand(row)} variant='outlined' disabled>
                            <Icon fontSize='1.125rem' icon='cryptocurrency:pay' />
                          </Button>
                        </TableCell>
                      </>
